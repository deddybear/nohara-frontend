...
export const Protofolio = () => {
  ...
  const handleChangeFiles = (e) => setPhotoPost(e.target.files[0]);
  const handleChangeName = (e) => setNamePost(e.target.value);
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios
            .delete(`${URL_BASE}/protofolio/delete/${id}`, {
              headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      data.append("photo", photoPost);
      data.append("name", namePost);

      await axios
        .post(`${URL_BASE}/protofolio/create`, data, {
          headers: headers,
        })
        .then((res) => {
          console.log(res);
  ...
  useEffect(() => {
    const fetchDataCaraousel = async () => {
      const url = `${URL_BASE}/protofolio/list?offset=${controller.offset}&limit=${controller.rowsPerPage}`;
      ...
      {loading ? (
        ...
        <div>
          ...
          <ModalComponent
            ...
            maxWidth={maxWidth}
            fullWidth={fullWidth}
            titlePost="Create New Post Protofolio"
          >
            <DialogContentText>
            ...
            <hr />
            <DialogContentText>Form Data Protofolio</DialogContentText>
            <Box
              component={"div"}
            ...
            >
              <FormControl sx={{ mt: 2 }}>
                <TextField
                  required
                  id="nama-protofolio"
                  label="Nama protofolio"
                  name="name_protofolio"
            ...
            <Box component={"div"} marginTop={2}>
              <p style={{ margin: 0 }}>File Photo Protofolio</p>
              <FormControl sx={{ mt: 2 }}>
                <Input
                  required
                  type={"file"}
                  id="standard-file-protofolio"
                  name="file_protofolio"
                  label="File Photos"
