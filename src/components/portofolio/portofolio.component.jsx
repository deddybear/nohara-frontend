...
const PortofolioComponent = ({dataList}) => (
    <div className='col-12 background-component text-white text-center py-5'>
            ...
            <Slider {...settings}>
                {
                    dataList.map(
                        ...
                        <div key={`portofolio_data_${index}`}>
                            <img className='w-100' src={`${process.env.REACT_APP_SERVER_URL}${item.path}`} alt={`portofolio_${index}`}></img>
                        </div>
                    )
                }
            </Slider>
        ...
        <div className='col-11 col-md-9 mx-auto bg-link-gallery rounded-3' 
                onClick={() => handleClickLink('https://gallery.noharaviscape.com/')}>
            <p className='text-dark text-link m-0 fs-6'>Gallery</p>
        </div>
    </div>
)
